...
//     - Checks whether the selected FIR band passes or rejects a 15 kHz tone
//
...
module Pdm_To_Pcm_tb ();
  ...
  // Tone frequency generated by both mic models.
  ...
  task automatic check_band(input string label, input bit expect_pass);
    begin
      ...
      check_pcm_band(error_count, label, expect_pass, pos_samples, pos_sample_time_ns,
                     ...
                     TONE_TOTAL_SAMPLES, TONE_CHECK_SAMPLES, TONE_FREQ_HZ, MIN_PASS_SWING,
  ...
  // evaluate whether that band passes or rejects the 15 kHz tone.
  ...
  initial begin
    ...
    // TEST 3
    // ----------------------------------------------------------
    run_band_check(3, BAND_10_18, "10-18 kHz band must get the expected tone.");

    // ----------------------------------------------------------
    // TEST 4
    // ----------------------------------------------------------
    // The same 15 kHz tone should be rejected by the 18-25 kHz band.
    run_band_check(4, BAND_18_25, "18-25 kHz band must get the expected tone.");

    // ----------------------------------------------------------
    // TEST 5
    // ----------------------------------------------------------
    run_band_check(5, BAND_25_32, "25-32 kHz band must get the expected tone.");

    // ----------------------------------------------------------
    // TEST 6
    // ----------------------------------------------------------
    run_band_check(6, BAND_32_40, "32-40 kHz band must get the expected tone.");
