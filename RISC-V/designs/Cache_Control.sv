...
// Description: Control logic for handling cache misses, including
//              interfacing with main memory to fill cache lines on a miss.
...
module Cache_Control (
    input logic clk,
    rst,  // Clock signal and active high reset signal
    ...
    output logic [TAG_WIDTH-1:0] tag_out,  // Output tag to rewrite upon a miss
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      tag_out <= '0;  // Reset the tag out to zero.
    end else if (clr_count) begin  // Clear the tag out register when we get a cache miss.
      tag_out <= {
        miss_address[CACHE_TAG_MSB:CACHE_TAG_LSB], 1'b1, 1'b0
      };  // Reset the valid count to zero.
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      main_memory_address <= {
        miss_address[31:4], 4'h0
      };  // Set the memory address to the first address of the block.
  ...
  always_ff @(posedge clk) begin
    if (rst) state <= IDLE;  // Reset into the idle state if machine is reset.
    else state <= nxt_state;  // Store the next state as the current state by default.
  end

  // Used as an SR flop to set the busy signal.
  always_ff @(posedge clk) begin
    if (rst) fsm_busy <= 1'b0;
  ...
  always_ff @(posedge clk) begin
    if (rst) mem_en <= 1'b0;
    else if (set_mem_en) mem_en <= 1'b1;
    else if (clr_mem_en) mem_en <= 1'b0;
  end
  ...
  always_comb begin
    ...
    case (state)
      SEND: begin  // SEND state - Fill the cache from main memory data
        ...
        if (eight_cycles) clr_mem_en = 1'b1;
      end
