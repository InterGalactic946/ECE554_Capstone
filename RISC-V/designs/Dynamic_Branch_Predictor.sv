...
module Dynamic_Branch_Predictor (
  ...
  state_t updated_prediction;  // The new prediction to be stored in the BHT on an incorrect pred.
  ...
  logic [DBP_BHT_ENTRY_W-1:0] BHT[BHT_ENTRIES];  // Declare BHT: {tag, prediction[1:0], valid}
  xlen_t BTB[BTB_ENTRIES];  // Declare BTB
  ////////////////////////////////////////////////
  ...
  always_ff @(posedge clk_i) begin
    if (rst_i) begin
      ...
      if (enable_i && wen_BHT_i) begin
        BHT[IF_ID_PC_curr_i[BHT_IDX_MSB:BHT_IDX_LSB]][DBP_BHT_ENTRY_W-1:3] <= IF_ID_PC_curr_i[XLEN-1:(BHT_IDX_MSB+1)];
        BHT[IF_ID_PC_curr_i[BHT_IDX_MSB:BHT_IDX_LSB]][2:1] <= updated_prediction;
        BHT[IF_ID_PC_curr_i[BHT_IDX_MSB:BHT_IDX_LSB]][0] <= updated_valid;
      end

      // Update BTB entry if needed (when a branch is taken)
      if (enable_i && wen_BTB_i) begin
        BTB[IF_ID_PC_curr_i[BTB_IDX_MSB:BTB_IDX_LSB]] <= actual_target_i;
      end
    end
  end
  ...
  assign write_tag = (enable_i) ? BHT[IF_ID_PC_curr_i[BHT_IDX_MSB:BHT_IDX_LSB]][DBP_BHT_ENTRY_W-1:3]
                                : '0;
