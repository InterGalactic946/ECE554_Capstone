...
module DynamicBranchPredictor_model (
    ...
    input logic [31:0] PC_curr,           // Current PC address
    input logic [31:0] IF_ID_PC_curr,     // Pipelined previous PC address
    ...
    input logic [31:0] actual_target,     // Actual target address for the branch (from the decode stage)
    ...
    output logic [31:0] predicted_target  // Predicted target address (from BTB)
);
  ...
  logic [31:0] BTB [0:BTB_ENTRIES-1];                // Declare BTB
  ////////////////////////////////////////////////
  ...
  always @(posedge clk) begin
      if (rst) begin
          ...
          if (enable && wen_BHT) begin
              BHT[IF_ID_PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][DBP_BHT_ENTRY_W-1:3] <= IF_ID_PC_curr[XLEN-1:(BHT_IDX_MSB+1)];  // Store the PC tag
              BHT[IF_ID_PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][2:1] <= updated_prediction;                                      // Store the 2-bit prediction along with the tag
              BHT[IF_ID_PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][0] <= updated_valid;                                             // Store the updated valid bit
          end

          // Update BTB entry if needed (when a branch is taken)
          if (enable && wen_BTB) begin
              BTB[IF_ID_PC_curr[BTB_IDX_MSB:BTB_IDX_LSB]] <= actual_target;  // Store the target address
          end
      end
  end

  // Get the valid bit of the branch.
  assign valid = (enable) ? BHT[PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][0] : 1'b0;

  // Asynchronously read out the prediction when enabled.
  assign prediction_rd = (enable) ? BHT[PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][2:1] : 2'h0;

  // Read out the tag stored in the memory when enabled for a read.
  assign read_tag = (enable) ? BHT[PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][DBP_BHT_ENTRY_W-1:3] : '0;

  // Compare the tags of the current PC and previous PC address in the cache to determine if they match.
  assign read_tags_match = (PC_curr[XLEN-1:(BHT_IDX_MSB+1)] == read_tag);
  ...
  assign predicted_target = (enable) ? BTB[PC_curr[BTB_IDX_MSB:BTB_IDX_LSB]] : '0;
  //////////////////////////////////////////
  ...
  assign write_tag = (enable) ? BHT[IF_ID_PC_curr[BHT_IDX_MSB:BHT_IDX_LSB]][DBP_BHT_ENTRY_W-1:3] : '0;

  // Check if the write tags match for the current IF_ID_PC and the previous PC address in the cache.
  assign write_tags_match = (IF_ID_PC_curr[XLEN-1:(BHT_IDX_MSB+1)] == write_tag);
