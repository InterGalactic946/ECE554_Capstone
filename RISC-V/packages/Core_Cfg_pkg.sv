...
package Core_Cfg_pkg;
    ...
    localparam int unsigned BHT_IDX_W = $clog2(BHT_ENTRIES); // Width of the BHT index (log2 of number of entries).
    localparam int unsigned BTB_IDX_W = $clog2(BTB_ENTRIES); // Width of the BTB index (log2 of number of entries).
    localparam int unsigned PC_ALIGN_LSB = $clog2(ILEN_BYTES); // Number of always-zero PC LSBs for aligned instruction fetch.
